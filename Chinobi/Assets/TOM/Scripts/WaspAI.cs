...
public class WaspAI : MonoBehaviour
{
    ...
    private float gravity; //force of gravity pulling the enemy down.
    private float antigravity; //force at which floating/flying enemies repel
    ...
    {
        if (estimateElevation == true && floatHeight > 0.0f)
        {
    ...
    bool TargetIsInSight()
    {
        ...
        if (Physics.Linecast(transform.position, target.position, out RaycastHit sight))
        {
        ...
        if (!canFly && floatHeight > 0.0f)
        {
            ...
            if (estimateElevation && estRayTimer > 0.0f)
            {
                if (Time.time > estGravityTimer)
                {
                    ...
                    if (Physics.Raycast(transform.position, -Vector3.up, out RaycastHit floatCheck))
                    {
            ...
            else
            {
                ...
                if (Physics.Raycast(transform.position, -Vector3.up, out RaycastHit floatCheck, floatHeight + 1.0f))
                {
